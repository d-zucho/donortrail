...
const Header = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      fetch(
        'http://127.0.0.1:7302/ingest/e8861fcc-1294-42d7-a788-e73e7f09fd62',
        {
          method: 'POST',
          headers: {
  ...
  return (
    <header
      id='header'
      className={cn(
        'py-5 sticky top-0 z-50',
        isScrolled &&
          'bg-background/70 border-b border-white/10 backdrop-blur-lg lg:px-5',
      )}
    >
      <MaxWidthWrapper>
        <div className={'flex items-center justify-between'}>
          ...
          <div className='flex items-center gap-5 shrink-0'>
            <Link href='/'>
              <span className='font-bold font-bebas-neue text-4xl text-primary'>
                Donor Trail
              </span>
            </Link>
              ...
              {NAV_LINKS.map((link) => (
                <Link
                  ...
                  className={buttonVariants({
                    variant: 'ghost',
                    className: `font-medium ${path === link.href ? 'text-primary bg-primary/5' : ''}`,
                  })}
                >
                  {link.label}
                </Link>
                  ...
                  <SignUpButton>
                    <button
                      className={buttonVariants({
                        variant: 'default',
                        className:
                          'px-5 py-5 min-w-24 font-medium relative group',
                      })}
                    >
